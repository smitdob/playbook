...
export interface FileValidationResult {
  filePath: string;
  fileName: string;
  fileType: 'Skills.md' | 'Validation.md' | 'tool' | 'unknown';
  agentName: string;
  issues: Issue[];
  passed: boolean;
}
...
export interface WorkspaceValidationResult {
  ...
  folderResults: FolderValidationResult[];
  passed: boolean;
}

// ── Helpers ──────────────────────────────────────────────────────────────────

export function extractAgentName(content: string): string {
...
function isSubAgent(folderPath: string): boolean {
  return folderPath.includes('subagents');
}

// ── Skills.md validator ──────────────────────────────────────────────────────

function validateSkillsFile(content: string, filePath: string): Issue[] {
  const issues: Issue[] = [];
  const isSub = isSubAgent(filePath);

  // ── 1. Required sections ─────────────────────────────────────────────────
  const sectionsToCheck = isSub
    ? [...SKILLS_REQUIRED_SECTIONS, ...SKILLS_SUBAGENT_REQUIRED_SECTIONS]
    : SKILLS_REQUIRED_SECTIONS;
  ...
  } else if (!PASCAL_CASE_AGENT_REGEX.test(agentName)) {
    issues.push({
      type: 'error',
      category: 'Naming Rule',
  ...
  if (content.includes('## Model Configuration')) {
    ...
    for (const field of MODEL_CONFIG_REQUIRED_FIELDS) {
  ...
  // ── 6. Completion signal ─────────────────────────────────────────────────
  const hasSignal = VALID_COMPLETION_SIGNALS.some(s => content.includes(s));
  if (!hasSignal) {
    issues.push({
      type: 'warning',
      category: 'Completion Signal',
      message: 'No completion signal found in Skills.md',
      fix: `Add a completion signal in System Message rules (e.g. REQUIREMENT_DONE, QA_DONE). Valid: ${VALID_COMPLETION_SIGNALS.slice(0, 5).join(', ')}...`,
    });
  }
...
function validateValidationFile(content: string): Issue[] {
  const issues: Issue[] = [];

  // ── 1. Required sections ─────────────────────────────────────────────────
  for (const section of VALIDATION_REQUIRED_SECTIONS) {
    if (!content.includes(section)) {
      issues.push({
        type: 'error',
        category: 'Missing Section',
...
export function validateAgentFolder(folderPath: string): FolderValidationResult {
  ...
  if (!PASCAL_CASE_AGENT_REGEX.test(folderName)) {
    issues.push({
      type: 'error',
      category: 'Naming Rule',
  ...
  if (!VALID_AGENT_CATEGORIES.includes(parentFolder) && parentFolder !== 'subagents') {
    issues.push({
      type: 'warning',
  ...
  if (fs.existsSync(subagentsFolderPath)) {
    ...
    for (const sub of subFolders) {
      ...
      if (!PASCAL_CASE_AGENT_REGEX.test(sub)) {
        issues.push({
          type: 'error',
...
export function validateSingleFile(filePath: string): FileValidationResult {
...
export async function validateWorkspace(
  ...
  const duplicateAgentNames: string[] = [];
  ...
  const totalErrors = folderResults.reduce(
    (sum, r) => sum + [...r.issues, ...r.files.flatMap(f => f.issues)].filter(i => i.type === 'error').length, 0
  );
  const totalWarnings = folderResults.reduce(
    (sum, r) => sum + [...r.issues, ...r.files.flatMap(f => f.issues)].filter(i => i.type === 'warning').length, 0
  );
