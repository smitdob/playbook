...
exports.showAiAssistPanel = showAiAssistPanel;
...
// ── Call OpenRouter API ───────────────────────────────────────────────────────
async function callOpenRouter(apiKey, messages, model = FREE_MODELS[0]) {
...
function detectContext(filePath) {
    ...
    if (parts.includes('CommonAgent'))
        category = 'CommonAgent';
    else if (parts.includes('MasterAgent'))
        category = 'MasterAgent';
    // Detect if sub-agent
    ...
    if (fs.existsSync(filePath)) {
        ...
        const extracted = (0, validator_1.extractAgentName)(content);
        if (extracted)
            agentName = extracted;
    }
    return {
...
function buildSkillsPrompt(ctx, currentContent) {
    return `${rules_1.AI_PROJECT_CONTEXT}
...
function buildValidationPrompt(ctx, currentContent, skillsContent) {
    return `${rules_1.AI_PROJECT_CONTEXT}
...
// ── AI Panel UI ───────────────────────────────────────────────────────────────
async function showAiAssistPanel(context, filePath) {
    // Get file path
    if (!filePath) {
    ...
    const validationResult = fs.existsSync(filePath)
        ? (0, validator_1.validateSingleFile)(filePath)
        : null;
    ...
    const panel = vscode.window.createWebviewPanel('sdlcAiAssistant', `AI Assistant — ${ctx.agentName}/${fileName}`, vscode.ViewColumn.Beside, { enableScripts: true, retainContextWhenHidden: true });
    // Build initial HTML
    panel.webview.html = getWebviewHtml(ctx.agentName, fileName, ctx.category, ctx.isSubAgent, ctx.parentAgent, errors, warnings);
    ...
    panel.webview.onDidReceiveMessage(async (message) => {
        if (message.command === 'askAI') {
            ...
            try {
                ...
                const systemPrompt = rules_1.AI_PROJECT_CONTEXT;
...
// ── Webview HTML ──────────────────────────────────────────────────────────────
function getWebviewHtml(agentName, fileName, category, isSubAgent, parentAgent, errors, warnings) {
    const errorItems = errors.map(e => `<li class="err">❌ <b>${e.category}:</b> ${e.message}<br><span class="fix">→ Fix: ${e.fix}</span></li>`).join('');
    const warningItems = warnings.map(w => `<li class="warn">⚠️ <b>${w.category}:</b> ${w.message}<br><span class="fix">→ Fix: ${w.fix}</span></li>`).join('');
